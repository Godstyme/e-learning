		...
		case 'uploadImage':
			if ($_SERVER["REQUEST_METHOD"] == "POST") {
				...
				elseif ($_FILES["fileToUpload"]["size"] > 3000000) {
					...
					$image = basename($_FILES["fileToUpload"]["name"]);
					move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file);
					$insertResponse = $insertData->uploadFile($username,$coursetitle,$image);
		...
		case 'uploadMedia':
			if ($_SERVER["REQUEST_METHOD"] == "POST") {
				...
				if($imageFileType != "mp3" && $imageFileType != "mp4" && $imageFileType != "wma") {
						$response = array('status'=>0,'input'=>"name",'message'=>"Sorry, only media files are allowed.");
						
				}
				// Check file size
				// elseif ($_FILES["fileToUpload"]["size"] > 3000000) {
				// 		$response = array('status'=>0,'input'=>"name",'message'=>"Sorry, THe media Is Too Large, Image Must Not Be More Than 3MB");
				...
				// }
					...
					$image = basename($_FILES["fileToUpload"]["name"]);
					move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file);
					$insertResponse = $insertData->uploadMedia($username,$coursetitle,$image);
